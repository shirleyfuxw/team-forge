...
**Status:** v0.4.0 — two archetypes (agent-team + workflow), both forge paths validated end-to-end. The forged dashboard is now a single self-contained, interactive explorer (warm-ivory design system, scroll-spy TOC, filter/sort, click-to-build copy-prompt footer) shared by both archetypes — one shell + one client-side renderer, written either by the monitor agent (team) or `gen_dashboard.py` (workflow); see [docs/specs/2026-06-23-playground-dashboard-migration.md](./docs/specs/2026-06-23-playground-dashboard-migration.md). Prior v0.3.0 retro #1687 fixes folded in (domain-named durable paths, naming discipline, teardown phase, dashboard/ledger hygiene). See [SCOPING.md](./SCOPING.md) (agent-team) + [WORKFLOW-SCOPING.md](./WORKFLOW-SCOPING.md) (workflow) for the designs, [tests/README.md](./tests/README.md) for what was validated.
