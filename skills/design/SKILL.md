...
description: |
  ...
  before producing the design.yaml the forge consumes.
---

# team-forge:design — machinery design (opt-in)
...
This phase produces `.claude/team-forge/<team>/design.yaml` — what the forge
...
The contract deliberately doesn't ask these; they are machinery concerns. Ask now:

1. **Archetype (work-shape) triage:**
   > "As the work proceeds, do the agents each hold distinct, evolving context they
   > defend across rounds (research debate, competing approaches) — or is it a
...
2. **Roster needs beyond work/verify/advise?** Domain experts, external-tool
   wrappers, anything project-specific.
3. **Tracking expectations?** State fields + dashboard panels the user wants —
   the contract's done_when status is always panel one.
4. **Token budget?** Hard ceiling or soft target; per-cohort if iterative.
...
> **Archetype branch.** If Step 0's triage chose `archetype: workflow`, run **"Workflow archetype —
...
agent gets the contract (and any legacy brainstorm/plan docs), but a different lens.
...
**Point `contract:` at the problem contract first** (both archetypes): the forge lints it
and DERIVES the `goal:` block — the lead's standing orders — from it (statement, checkable
done_when, decision split). Never hand-compose `goal:` when a contract exists; a hand-written
block is only for contract-less legacy designs, and drift between the two is a defect.
...
4. **Purpose-anchored** — traceable to `project.domain` + `brief` + the contract's
...
domain/purpose (from the contract) so the design agents see only
...
> "Read the contract (+ any legacy brainstorm/plan docs). Propose a roster with at least one teammate per required
...
> "Read the contract + this draft roster. Validate:
...
> Flag anywhere the proposed roster lacks an agent that the milestones' interfaces would require."

Returns: validation issues + tracking spec proposal.

#### Lens 3: domain fit + risks

> "Read the contract + this draft roster + this skill catalog. Where
> does the roster feel 'forced'? Are there project-domain constraints from the
> contract that no roster entry addresses? Propose constraints[] entries for
...
5. **constraints**: take Lens 3's proposal; the contract's context + domain facts should map here
