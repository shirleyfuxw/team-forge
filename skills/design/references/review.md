...
| 2 | Role coverage | At least one teammate per required role (work/verify/advise, via `role` or `combined_roles`) AND exactly one `orchestrator`. tracker/monitor are OPTIONAL — absent means lead-written ledger + emitted `gen_dashboard.py` render step; if the roster DOES include them, flag whether the tracking load actually justifies standing agents. | **Yes** — the brand rule |
