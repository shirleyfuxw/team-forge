...
# · --sync-goal (re-derive goal_directive into a LIVE ledger; writes only that key).
...
def gap_is_promoted(target_repo, name):
    """A skill_gaps entry graduates when a human promotes its draft to .claude/skills/<name>/.
    One definition, shared by emission (don't re-emit a draft for a promoted skill), --resync
    (never overwrite human-owned promoted skills) and gate backing."""
    return (target_repo / ".claude" / "skills" / name / "SKILL.md").exists()
...
def emit_skill_gap_scaffolds(design, hub_dir, target_repo, generated):
    ...
    for gap in gaps:
        ...
        if gap_is_promoted(target_repo, gap['name']):
...
PLAN_TASK_FIELDS = ('id', 'name', 'output', 'depends_on', 'blast_radius', 'gate_set', 'dispatch')


def bake_plan(design, target_repo):
    """The DESIGN-DERIVED half of the ledger — everything the runtime needs to know about the
    work, mirrored from design.yaml so `status.json` is the runtime's ONLY read
    (WORKFLOW-SCOPING.md, settled decision 3).

    Kept as its own top-level `plan` block precisely so ownership is legible and --resync is
    unambiguous: `plan` is regenerated wholesale from design.yaml and is never hand-edited;
    every sibling key is live state and is never touched. Folding these definitions into the
    live `tasks[]` entries instead would force a field-level merge on every resync — which is
    exactly the ambiguity that produced issue #36, moved into a new file."""
    plan = {'gates': design['gates']}
    tasks = design.get('tasks') or []
    if tasks:
        plan['tasks'] = [{k: t[k] for k in PLAN_TASK_FIELDS if k in t} for t in tasks]
    if 'queue' in design:
        plan['queue'] = design['queue']
    # Gate backing, ADDITIVE (gates keeps its {name: criterion} shape so TASKS.yaml stays a
    # faithful mirror of design.yaml). "Gates that call an unpromoted skill fail-closed" was
    # asserted in four places and implemented nowhere — the lead had no way to know a gate was
    # unbacked short of an opaque invocation failure. Now it can refuse and say why.
    backing = {g['backing']: {'skill': g['name'],
                              'promoted': gap_is_promoted(target_repo, g['name'])}
               for g in (design.get('skill_gaps') or []) if g.get('backing') in design['gates']}
    if backing:
        plan['gate_backing'] = backing
    return plan


def render_tasks_yaml(design, team):
    """TASKS.yaml — a DERIVED, tracked build artifact: the work list in reviewable form, for
    humans and PR diffs. The runtime never reads it (settled decision 3); it reads the `plan`
    block baked into tracker/status.json. Regenerated by --resync like any other derived file."""
    doc = {'gates': design['gates']}
    if 'tasks' in design: doc = {'tasks': design['tasks'], **doc}
    if 'queue' in design: doc['queue'] = design['queue']
    return (f"# TASKS.yaml — work list for the {team} workflow, DERIVED from design.yaml.\n"
            "# Not runtime state and not lead-editable: the runtime reads the `plan` block in\n"
            "# tracker/status.json. Edit design.yaml, then `forge.py <design> --resync`.\n\n"
            + yaml.safe_dump(doc, sort_keys=False, allow_unicode=True))


def initial_status_json_workflow(design, target_repo):
    state = {}
    for s in design['ledger']['state_shape']:
        state[s['id']] = {'string': None, 'int': 0, 'float': 0.0, 'bool': False, 'list': [], 'object': {}}[s['type']]
    # `plan` = design-derived (regenerated by --resync); every sibling key below is live state.
    state['plan'] = bake_plan(design, target_repo)
...
def forge_workflow(design):
    ...
    # TASKS.yaml — derived, tracked, runtime-invisible (settled decision 3).
    (hub_dir / "TASKS.yaml").write_text(render_tasks_yaml(design, team))
...
# Two kinds of staleness, reported separately. TEMPLATE drift: baked copies (launcher,
# profiles, gen_dashboard.py) fall behind the plugin's templates. DESIGN drift: design.yaml
# changed since the forge, so the derived artifacts — TASKS.yaml and the ledger's `plan`
# block — no longer mirror it. --resync fixes both while preserving live state; --check
...
def _regen_content(design, team, basename, fmeta, target_repo):
    ...
    if kind == 'skill_gap_scaffold':
        # A DRAFT is derived output and must track its skill_gaps spec — sharpening a spec
        # otherwise left the scaffold frozen at forge-time text, the same silent staleness
        # as #36 but on the deliverable. A PROMOTED skill is human-owned: never touched.
        name = Path(fmeta['path']).parent.name
        if gap_is_promoted(target_repo, name):
            return None
...
def _plan_delta(old, new):
    """What a re-bake would change, in words. The false-clean fix needs to SAY what is stale —
    reporting only that *something* differs is what let "already current" read as "nothing to do"."""
    out = []
    og, ng = old.get('gates') or {}, new.get('gates') or {}
    if og != ng:
        add, rm = sorted(set(ng) - set(og)), sorted(set(og) - set(ng))
        mod = sorted(k for k in set(og) & set(ng) if og[k] != ng[k])
        bits = "".join([f" +{','.join(add)}" if add else "",
                        f" -{','.join(rm)}" if rm else "",
                        f" ~{','.join(mod)}" if mod else ""])
        out.append(f"gates: {len(og)} → {len(ng)}{bits}")
    ot, nt = old.get('tasks') or [], new.get('tasks') or []
    if ot != nt:
        oid, nid = [t.get('id') for t in ot], [t.get('id') for t in nt]
        add, rm = [i for i in nid if i not in oid], [i for i in oid if i not in nid]
        bits = "".join([f" +{','.join(add)}" if add else "",
                        f" -{','.join(rm)}" if rm else ""])
        out.append(f"tasks: {len(ot)} → {len(nt)}{bits}" + ("" if bits else " (definitions changed)"))
    ob, nb = old.get('gate_backing') or {}, new.get('gate_backing') or {}
    if ob != nb:
        flipped = [g for g in set(ob) & set(nb) if ob[g].get('promoted') != nb[g].get('promoted')]
        out.append("gate backing: " + ", ".join(
            sorted(f"{g}→{'promoted' if nb[g]['promoted'] else 'DRAFT'}" for g in flipped)
            or [f"{len(ob)} → {len(nb)} gate(s) with a skill dependency"]))
    oq, nq = old.get('queue') or {}, new.get('queue') or {}
    if oq != nq:
        add, rm = sorted(set(nq) - set(oq)), sorted(set(oq) - set(nq))
        bits = "".join([f" +{','.join(add)}" if add else "",
                        f" -{','.join(rm)}" if rm else ""])
        out.append(f"queue: {len(oq)} → {len(nq)} keys{bits}")
    return out


def resync_ledger_plan(design, target_repo, hub_dir, do_write):
    """Re-bake the ledger's design-derived `plan` block, preserving every live sibling key.

    This is the ONLY operation that propagates a design change into a RUNNING workflow, which
    is what #36 found missing. status.json is a regenerable cache over design.yaml: losing it
    costs progress, never the plan. Returns a list of human-readable deltas ([] = current)."""
    if design.get('archetype') != 'workflow':
        return []                                   # team archetype: milestones, no gates/queue
    p = hub_dir / "tracker" / "status.json"
    if not p.exists():
        return []                                   # never forged here, or torn down
    live = json.loads(p.read_text())
    fresh = bake_plan(design, target_repo)
    if live.get('plan') == fresh:
        return []
    delta = _plan_delta(live.get('plan') or {}, fresh) or ["plan block seeded"]
    if do_write:
        live['plan'] = fresh                        # every other key is live state — untouched
        p.write_text(json.dumps(live, indent=2))
    return delta


def _goal_delta(old, new):
    """What a goal sync changes, in words — the lead's standing orders should never move silently."""
    out = []
    if (old.get('statement') or '').strip() != (new.get('statement') or '').strip():
        out.append("statement rewritten")
    od, nd = old.get('done_when') or [], new.get('done_when') or []
    if od != nd:
        add, rm = [d for d in nd if d not in od], [d for d in od if d not in nd]
        out.append(f"done_when: {len(od)} → {len(nd)} (+{len(add)}/-{len(rm)})")
    for key in ('lead_decides', 'user_decides'):
        o, n = old.get(key) or [], new.get(key) or []
        if o != n:
            out.append(f"{key}: {len(o)} → {len(n)}")
    return out
...
def sync_goal(design, target_repo, team):
    ...
    preserves live state, and a full forge would destroy it. So after a contract revision the
    runtime kept executing its forge-time orders, invisibly."""
    p = target_repo / f".claude/team-forge/{team}/tracker/status.json"
    if not p.exists():
    ...
    if old == new:
        print(f"✓ {team}: goal_directive already matches the contract — no change.")
        return
    delta = _goal_delta(old, new) or ["directive replaced"]
    live['goal_directive'] = new
    live.setdefault('events', []).append(
        {'kind': 'goal_revised', 'iso': _NOW, 'delta': delta})
    p.write_text(json.dumps(live, indent=2))     # only goal_directive + the event log move
    print(f"✓ {team}: goal_directive re-derived from the contract; `goal_revised` logged.")
    for d in delta:
        print(f"   ↻ {d}")


def resync(design, target_repo, team, basename, do_write):
    """Land template AND design changes on a forged team, preserving live state.

    Reports the two staleness axes separately: template drift (baked copies behind the
    plugin) and design drift (derived artifacts behind design.yaml). do_write=False is
    --check, which reports without touching anything."""
    hub_dir = target_repo / f".claude/team-forge/{team}"
    ...
    plan_delta = resync_ledger_plan(design, target_repo, hub_dir, do_write)

    print(f"Team {team}: forged at {old_ver}, current templates {FORGE_VERSION} — "
          f"{'STALE' if old_ver != FORGE_VERSION else 'version matches'}")
    if do_write:
        # The hub copy is what --check/--resync read next time, so it has to track the design
        # they were just run against — otherwise the drift signal never clears.
    ...
    if design_drifted or plan_delta:
        ...
        print(f"⚠ design.yaml changed since forge (hash mismatch) — ledger `plan` {verb}:"
              if design_drifted else f"⚠ ledger `plan` {verb}:")
        for d in plan_delta:
            print(f"   ↻ {d}")
        if not plan_delta:
            print("   (design edit did not change any derived section)")
    gaps = design.get('skill_gaps') or []
    if gaps:
        draft = [g['name'] for g in gaps if not gap_is_promoted(target_repo, g['name'])]
...
# Contract revised → re-derive the runtime's standing orders. Touches one key; never emits.
if SYNC_GOAL:
    sync_goal(design, target_repo, team)
    sys.exit(0)
